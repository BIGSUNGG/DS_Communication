...
description: DS_Communication 리뷰 차원 — 구조. 코드 변경 시 레이어링·의존성·SOLID·과잉 추상화를 검토하는 reviewer용 기준. reviewer 서브에이전트가 diff 검토 시 참조한다.
---
...
1. **SOLID 위반** — `Document/00-AI/CONVENTIONS.md` 기준. SRP(한 클래스 두 책임), OCP(수정 폐쇄), LSP·ISP·DIP 순으로 점검.
...
5. **배치·네이밍 규약** — 파일 위치, 네임스페이스, 타입/멤버 명명이 `Document/00-AI/CONVENTIONS.md`와 일치하는지.
6. **문서 반영** — 구조가 바뀌었는데 `Document/02-Architecture/` 문서에 미반영이면 구조 이슈로 함께 지적.

## Pitfalls

- 문서화되지 않은 구조를 요구하지 말고, 현재 문서화된 규약(`Document/01-Overview/`, `Document/02-Architecture/`)만 근거로 삼는다.
