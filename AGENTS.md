# DS_Communication — AI 협업 규칙

이 파일은 pi가 모든 세션에 자동 로드한다. 아래 규칙은 사용자의 명시적 요청으로 채택되었다 (배경: `Document/05-Decisions/0010-harness-switch-to-pi.md`).
...
- 정체성: 연결형 통신 전송 계층 라이브러리(TCP·RUDP). 상용 Unity/.NET 게임 서버·클라이언트 공용.
- 재구축 상태: 이 리포는 전송 계층(직렬화 협력) 중심으로 재편 중이다. 기존 네트워크 스택과 문서는 `Legacy/` 아카이브 — 명시적 요청이 아니면 `Legacy/`를 활성 진실 공급원으로 취급하지 않는다.
- 활성 소스: `Source/` (Shared·TCP·RUDP 등). 세부 현 상태·스택: `Document/00-AI/CONTEXT.md`.
