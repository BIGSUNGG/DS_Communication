...
| F2-6 | 리소스 정리 | Session/파이프라인 Dispose; 끊김 후 송신 시도는 예외 또는 무시(결정: 구현 시 문서화) |
...
| F3-3 | 백프레셔 | 송신/Handler 큐 상한(`MaxPendingMessages`, 레거시 기본 10_000) |
| F3-4 | 핸들러 디스패치 | 타입→핸들러 등록 디스패치; **미등록 타입은 skip**(예외 아님) |
| F3-5 | 디스패치 모드 | 내부 큐 + 전용 처리 / `InlineDispatch`(수신 경로 동기 디스패치) 선택 |
