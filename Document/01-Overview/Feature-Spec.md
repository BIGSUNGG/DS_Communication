...
| F3-3 | 백프레셔 | 송신/Handler 큐 상한(`MaxPendingMessages`, 기본 `10_000`); 상한 도달 시 **공간 날 때까지 비동기 대기**. 메시지 채널(`IMessageChannel`) 수신은 **슬롯 대기(메시지 보유)까지 상한에 포함**해 강제하고, 초과 시 `DisconnectReason.FlowControl`로 **실패 폐쇄 단절** — 무제한 누적 없음 (바이트 채널은 수신 루프가 추가 읽기를 멈추는 백프레셔 유지) |
| F3-4 | 핸들러 디스패치 | 타입→핸들러 등록 디스패치; 정확 타입 미등록 시 **등록된 베이스 타입(상속·인터페이스) 중 가장 구체적인 것으로 폴백**, 맞는 핸들러가 전혀 없을 때만 **skip**(예외 아님); 핸들러 `Action` 예외는 **Trace 후 수신 루프 계속** |
| F3-5 | 디스패치 모드 | 내부 큐 / `InlineDispatch`(수신 경로 동기 디스패치) 선택 — 기본 **내부 큐**. **메시지 단위 채널(`IMessageChannel`) 경로는 `InlineDispatch`를 무시하고 항상 큐 강제**(수신 콜백이 세션 간 공유 폴링 스레드 — 느린 핸들러로 인한 타세션 차단 방지) |
...
| F3-10 | 프레임 길이 상한 | `MaxFrameLength`(기본 `4MB`, 절대 상한 `64MB`) — **바이트 경로**: 프레이머가 초과 프레임 거부(단절), 송신은 해당 항목만 격리. **메시지 경로(RUDP)**: 송신은 보내기 전 항목 격리, 수신은 역직렬화 전 거부(단절) — 양 경로 동일 계약 |
