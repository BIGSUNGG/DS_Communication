...
**조건부 사용 가능.** TCP(+옵션 TLS) 스택은 상용 전용 서버 투입 가능 수준. RUDP는 기밀성이 없어 **공개망 클라이언트 직결엔 부적합**(신뢰망·VPN 전제). 단, 실서비스 소크·부하 실측이 없어 "몇 접속까지, 어떤 처리량"에 대한 근거는 부재 — 점진적 트래픽 노출로 검증하며 투입해야 한다.

## 검증 근거 (직접 확인)

| 항목 | 결과 |
| ------ | ------ |
| 테스트 | 135/135 통과 (net10.0, 20초) — 고갈 공격 회귀 2건 포함 |
...
1. **소크 부재** — 24시간+ 장기 실행(누수, 재접속 폭주, 포트 재사용) 미검증. 감사 문서도 잔여 리스크로 인정.
...
5. **RUDP 기밀성 없음** — 평문 UDP + 키 없는 CRC. 공개망 클라이언트 통신은 TCP TLS 또는 VPN 위 운용.
