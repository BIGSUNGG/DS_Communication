...
| 항목 | 결과 |
| ------ | ------ |
| 테스트 | 148/148 통과 (net10.0, 26초) — 고갈 공격 회귀 2건 + 폭풍 청urn 소크 2건(TCP 16동시×3 wave RST/FIN 혼합·RUDP 상한 거부 혼합·포트 재바인딩) 포함 |
...
1. **소크 부재(부분 완화)** — 자동화된 청urn 소크(동시 접속 폭풍·RST/FIN 혼합 이탈·포트 재사용·슬롯 회복)는 테스트 스위트로 들어왔으나(2026-09-09, 148테스트), **24시간+ 장기 실서비스 소크**(누수·재접속 폭주의 실트래픽 검증)는 여전히 미수행 — 스테이징 실측 필요.
