...
| ✅ 옵션(RUDP TLS) | **`RudpTransportOptions.Tls`**(DTLS 1.2, BouncyCastle — 2.5.0+) — 연결 확립 후 핸드셰이크 선행 완료(상한 15초, 실패·초과 폐기+슬롯 회수). 서버 `ServerCertificate`(개인 키 포함), 클라는 **핀닝(`RemoteCertificateValidation`) 권장** — `TargetHost` 이름 일치는 2.7.0부터 `AllowNameOnlyCertificateMatch=true` 옵트인 필요(기본 거부 — 자체서명 인증서 MITM 차단, 옵트인해도 유효기간 검사 강제). 검증 수단 없으면 기본 거부. 16,381B 초과 메시지는 `ReliableOrdered`만(청킹·재조립 64MB 상한, 위반 fail-closed). **양단 같은 설정 필요**(와이어 비호환) — [[../05-Decisions/0009-rudp-tls-dtls | ADR 0009]] |
