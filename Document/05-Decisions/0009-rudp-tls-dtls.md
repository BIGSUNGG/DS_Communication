...
3. **클라이언트 검증은 fail-closed** — OS 인증서 스토어 검증 계약이 없으므로 `RemoteCertificateValidation`(핀닝, 게임 표준 경로) 또는 `TargetHost`(SAN/CN 일치) 중 하나를 반드시 설정한다. 둘 다 없으면 서버 인증서를 **기본 거부**한다. 무조건 통과 콜백 금지는 TCP와 동일 원칙. **2.7.0 강화([[../01-Overview/Commercial-Risk-Scan|Commercial-Risk-Scan]] R2)**: 이름 일치는 신뢰 체인 없이 CN만 비교해 같은 이름의 자체서명 인증서로 중간자가 통과할 수 있어 `AllowNameOnlyCertificateMatch=true` 옵트인이 있어야만 수용한다(기본 거부 — 공개 동작 변화, 마이너) — 옵트인해도 유효기간(NotBefore/NotAfter) 검사는 강제한다.
...
- 2.7.0: TargetHost 옵트인 회귀 3종 추가(옵트인 미설정 거부·옵트인+만료 인증서 거부·옵트인+유효 인증서 성공, 152 → 154). 기존 일치/불일치 테스트는 옵트인 경로로 갱신.
