...
| 패키지 | TFM (목표) | 의존 | 상태 |
| -------- | ------------ | ------ | ------ |
| `Communication.Shared` | netstandard2.1 | (없음) | 구현 완료 — 테스트 포함, 2.3.0 배포 |
| `Communication.Network.TCP.Shared` | netstandard2.1 | Shared | 구현 완료 — 2.3.0 배포 |
| `Communication.Network.TCP.Server` | netstandard2.1 | TCP.Shared | 구현 완료 — 2.3.0 배포 |
| `Communication.Network.TCP.Client` | netstandard2.1 | TCP.Shared | 구현 완료 — 2.3.0 배포 |
| `Communication.Network.RUDP.Shared` | netstandard2.1 | Shared + **LiteNetLib 2.1.4** | 구현 완료 — 테스트 포함, 2.3.0 배포 |
| `Communication.Network.RUDP.Server` | netstandard2.1 | RUDP.Shared | 구현 완료 — 2.3.0 배포 |
| `Communication.Network.RUDP.Client` | netstandard2.1 | RUDP.Shared | 구현 완료 — 2.3.0 배포 |
...
- 자체 RUDP는 이후 **별 프로젝트**
- 배포: GitHub Actions `nuget-publish.yml` — `v*` 태그 → Communication.Shared, `tcp/v*` 태그 → TCP 3종, `rudp/v*` 태그 → RUDP 3종 (2.3.0 배포 완료)
